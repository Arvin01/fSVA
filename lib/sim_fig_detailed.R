# Code to create figures for simulation
sim_fig_detailed<-function(object,maintitle=""){
	...
	for(i in 1:len1){
		...
		# create a blank plot
		plot(x,seq(0.1,1,by=.1),pch=".",col="white",xlab="Correlation between batch and outcome",
			 ylab="Prediction Accuracy",main=maintitle,ylim=c(0.3,1))
			 
		# plot everything
		lines(x=x,y=y1,col=cols1[2],lwd=3)
		#lines(x=x,y=y2,col=2,lwd=3)
		lines(x=x,y=y3,col=cols3[2],lwd=3)
		lines(x=x,y=y4,col=cols4[2],lwd=3)
		lines(x=x,y=y5,col=cols5[2],lwd=3)
